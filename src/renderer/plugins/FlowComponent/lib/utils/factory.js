 ...
 * @LastEditors: 羊驼
 * @LastEditTime: 2024-07-10 09:42:52
...
export default class TypeFactory {
   ...
   * @param {function} openSetting 打开编辑器时的操作
   * @param {function} mounted 流程图显示时的初始化Node函数（可用于提交或者获取全局的数据）
   * @return {*}
   */
  createNode(type, setting, textSetting, saveSetting, openSetting, mounted, nodeName = "节点", filename = "Normal", form = "NodeForm") {
    if (type === undefined) {
    ...
    return {
      ...
      handleOpen(nodeConfig) {
        let nodeSetting = nodeConfig.setting;
        return openSetting && openSetting(nodeSetting);
      },
      mounted(nodeConfig) {
        let nodeSetting = nodeConfig.setting;
        return mounted && mounted(nodeSetting);
      },
    };
  }
  ...
  /**
   * @description: 获取编辑前处理方式
   * @param {*}
   * @return {*}
   */
  getEditorOpenHandle(type, nodeConfig) {
    return this.factory[type] && this.factory[type].handleOpen && this.factory[type].handleOpen(nodeConfig);
  }

  /**
   * @description: 获取初始化生命周期
   * @param {*}
   * @return {*}
   */
  getNodeMounted(type, nodeConfig) {
    return this.factory[type] && this.factory[type].mounted && this.factory[type].mounted(nodeConfig);
  }
  ...
  batchRigster(custom) {
    ...
    for (let { viewComponent, nodeComponent, nodeName = "节点", type, setting, textHandle, saveHandle, openHandle, mounted } of custom) {
      ...
      let node = this.createNode(value, setting, textHandle, saveHandle, openHandle, mounted, nodeName, filename, viewComponent.name);
   ...
   */
  getNodeType() {
    return JSON.parse(JSON.stringify(this.nodeType));
  }
}
