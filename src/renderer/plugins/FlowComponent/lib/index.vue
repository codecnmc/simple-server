...
export default {
  ...
  methods: {
    ...
    reset() {
      ...
      this.init && this.nodeMounted();
    },
    // 读取数据的时候触发所有节点的mounted函数 也可以自己手动触发
    nodeMounted() {
      let nodes = this.getFlatDic();
      for (let id in nodes) {
        let node = nodes[id];
        this.$factory.getNodeMounted(node.type, node);
        node.error = this.$factory.getTypeBeforeSave(node.type, node);
      }
      this.refresh = false;
      this.$nextTick(() => {
        this.refresh = true;
